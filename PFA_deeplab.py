...
def main():
    ...
    # ===================== Model ======================
    # ==================================================
    part0_teacher = deeplab.modeling.__dict__[args.model](num_classes=part0_num_classes, output_stride=args.output_stride)
    part1_teacher = deeplab.modeling.__dict__[args.model](num_classes=part1_num_classes, output_stride=args.output_stride)
    student = deeplab.modeling.__dict__[args.model](num_classes=part0_num_classes+part1_num_classes, output_stride=args.output_stride)
    ...
    netG = engine.models.generator.DeepGenerator(nz=args.z_dim, nc=3, img_size=256)
    netD = engine.models.generator.DeeperPatchDiscriminator(nc=3, ndf=64)
    
    part0_teacher.load_state_dict(torch.load('checkpoints/pretrained/%s_%s.pth'%(args.dataset0, args.model))['model_state'])
    part1_teacher.load_state_dict(torch.load('checkpoints/pretrained/%s_%s.pth'%(args.dataset1, args.model))['model_state'])
