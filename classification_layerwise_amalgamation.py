...
#     car_val_dst.transform = aircraft_val_dst.transform = val_transform
    ...
    train_dst = torch.utils.data.ConcatDataset( [part0_train, part1_train] )
    train_loader = torch.utils.data.DataLoader( train_dst, batch_size=128, shuffle=True, num_workers=4 )
    part0_val_loader = torch.utils.data.DataLoader( part0_val, batch_size=128, shuffle=False, num_workers=4 )
    part1_val_loader = torch.utils.data.DataLoader( part1_val, batch_size=128, shuffle=False, num_workers=4 )
    ...
    # ==================== Trainer =====================
    # ==================================================
    output_dir = 'run/layerwise_ka-%s'%( time.asctime().replace( ' ', '_' ) )
    trainer = LayerWiseAmalgamator( 
        logger=get_logger(name='layerwise-ka', output= os.path.join(output_dir, 'log.txt')), 
